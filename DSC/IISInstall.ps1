...
Param ( [string] $nodeName, $WebDeployPackagePath, $connectionString )
...
Node $nodeName
  {
    ...
    {
        TestScript = {$false}
        SetScript = {[Environment]::SetEnvironmentVariable("connectionString", "$using.connectionString",[EnvironmentVariableTarget]::User)}
